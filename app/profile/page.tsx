...
function ProfileContent() {
  ...
  console.log("ALLLY CHECK THIS STUFF OUT");
  ...
  return (
    <ErrorBoundary fallback={<div className="p-4 text-red-500">Failed to load subscription details. Please try refreshing.</div>}>
      <div className="min-h-screen bg-surface-light dark:bg-surface-dark p-8 max-w-4xl mx-auto">
        {paymentStatus === "success" && (
          <div className="mb-8 p-4 bg-green-50 dark:bg-green-900/30 rounded-lg">
            <p className="text-green-600 dark:text-green-400">🎉 Thank you for your subscription! Your payment was successful.</p>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-8">
          ...
          {error ? (
            ...
            <div className="space-y-2">
              <p>
                <span className="font-medium">Status:</span> <span className={`${subscription.status === "active" ? "text-green-500" : "text-yellow-500"}`}>{subscription.status.charAt(0).toUpperCase() + subscription.status.slice(1)}</span>
              </p>
              <p>
              ...
              {subscription.status === "canceled" ? (
                ...
                <div className="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg">
                  <p className="text-yellow-600 dark:text-yellow-400 mb-2">Your subscription will end on {new Date(subscription.current_period_end).toLocaleDateString()}</p>
                  <button
            ...
            <div className="mt-4 space-y-4">
              {isInTrial ? (
                <>
                  <p className="text-yellow-600 dark:text-yellow-400">You are currently in your 48-hour trial period. Your trial will end on {trialEndTime ? new Date(trialEndTime).toLocaleDateString() : "soon"}.</p>
                ...
                <>
                  <div className="p-4 bg-red-50 dark:bg-red-900/30 rounded-lg mb-4">
                    <p className="text-red-600 dark:text-red-400">Your trial period ended on {new Date(trialEndTime).toLocaleDateString()}.</p>
                    <p className="mt-2">Subscribe now to regain access to the Patient Educator Report Generator.</p>
                  </div>
                </>
              ) : (
                <p>Subscribe to unlock the patient educator report generator.</p>
              )}
